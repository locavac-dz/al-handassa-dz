...
const adminApp = {
  ...
  async init() {
    ...
    document.getElementById('admin-password')?.addEventListener('keydown', e => {
      if (e.key === 'Enter') adminApp.doLogin();
    });
    // Si déjà connecté, connexion automatique
    const token = localStorage.getItem('hds_token');
    if (token) {
      try {
        const res = await get('/auth/me');
        if (res.user?.role === 'admin') { this._openApp(res.user); return; }
      } catch(e) {}
    }
    // Sinon afficher le formulaire prêt
    const btn = document.getElementById('guard-btn');
    if (btn) { btn.disabled = false; btn.innerHTML = '<i class="fas fa-lock"></i> Se connecter'; }
  },

  async doLogin() {
    const email    = document.getElementById('admin-email')?.value.trim();
    const password = document.getElementById('admin-password')?.value;
    const msg      = document.getElementById('guard-msg');
    const btn      = document.getElementById('guard-btn');
    ...
    try {
      ...
      localStorage.setItem('hds_token',   data.access_token);
      localStorage.setItem('hds_refresh', data.refresh_token);
      localStorage.setItem('hds_user',    JSON.stringify(data.user));
      this._openApp(data.user);
    } catch(e) {
      msg.textContent = e.message;
      btn.disabled = false;
      btn.innerHTML = '<i class="fas fa-lock"></i> Se connecter';
    }
  },
  ...
  async _loadUsers() {
    try {
      ...
      // Utiliser les recent_users du dashboard pour démarrer, puis charger via autre endpoint si dispo
      const users = res.recent_users || [];
      ...
      wrap.innerHTML = `
        ...
        <div style="text-align:center;padding:12px;color:#94a3b8;font-size:0.82rem">
          Affichage des 8 derniers inscrits — ${res.total_users} utilisateurs au total
        </div>`;
    } catch(e) { toast(e.message,'err'); }
  },
  ...
  _filterUsers() {
    ...
    document.querySelectorAll('#users-table tbody tr').forEach(tr => {
      tr.style.display = (!search || tr.dataset.search?.includes(search))
                      && (!role   || tr.dataset.role === role) ? '' : 'none';
    });
  },
        ...
        <table class="adm-table" id="vid-table">
          ...
          <tbody>${videos.map(v=>`
            <tr data-search="${v.title.toLowerCase()}">
              ...
              <td>
                <label class="toggle">
                  <input type="checkbox" ${v.is_active?'checked':''} onchange="adminApp.toggleVideo('${v.id}',this.checked)">
                  <span class="toggle-slider"></span>
                </label>
              </td>
              <td>
                <button class="btn-adm btn-adm-blue" onclick="adminApp.editVideo(${JSON.stringify(v).replace(/"/g,'&quot;')})">
                  <i class="fas fa-edit"></i>
                </button>
                <button class="btn-adm btn-adm-red" onclick="adminApp.deleteVideo('${v.id}','${v.title.replace(/'/g,'')}')">
                  <i class="fas fa-trash"></i>
                </button>
              </td>
    ...
    this.openModal(isEdit ? '✏️ Modifier la vidéo' : '🎬 Ajouter une vidéo', `
      <div class="adm-form">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
          <div class="adm-field"><label>ID YouTube *</label>
            <input type="text" id="vf-ytid" value="${v.youtube_id||''}" placeholder="Ex: dQw4w9WgXcQ"/></div>
          <div class="adm-field"><label>Durée (secondes)</label>
            <input type="number" id="vf-dur" value="${v.duration_sec||''}" placeholder="Ex: 360"/></div>
        </div>
  ...
  async saveVideo(id) {
    const payload = {
      ...
      youtube_id:  document.getElementById('vf-ytid').value.trim(),
      duration_sec:parseInt(document.getElementById('vf-dur').value)||null,
    ...
    if (!payload.title || !payload.youtube_id) { toast('Titre et ID YouTube requis', 'err'); return; }
    try {
