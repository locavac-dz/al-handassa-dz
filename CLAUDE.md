...
- **Connu et non traité** : CSP Helmet bloquant les `onclick` inline quand le front est servi par Express ; `rejectUnauthorized:false` (SMTP, base) ; nodemailer ≤ 9.1 (correctif en 10.x) ; Dockerfile/railway.json/nginx à revoir ; `.git` alourdi par ~2 Go d'objets orphelins (`git prune` + `git gc`) et `backend/uploads/ecotec` suivi par Git ; `POST /api/products/:id/reviews` n'exige pas d'avoir acheté le produit ; sitemap.xml périmé.
